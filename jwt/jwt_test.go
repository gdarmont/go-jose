...
func TestDecodeToken(t *testing.T) {
	...
	assert.EqualError(t, tok.Claims([]byte("invalid-secret")), "gdarmont/go-jose: error in cryptographic primitive")
	...
	assert.EqualError(t, tok.Claims(&testPrivRSAKey2.PublicKey), "gdarmont/go-jose: error in cryptographic primitive")
	...
	assert.EqualError(t, err, "gdarmont/go-jose: compact JWS format must have three parts")
	...
	assert.EqualError(t, tok4.Claims([]byte("invalid-secret-key")), "gdarmont/go-jose: error in cryptographic primitive")
	...
	assert.EqualError(t, tok5.Claims(testPrivRSAKey2), "gdarmont/go-jose: error in cryptographic primitive")
	...
	assert.EqualError(t, err, "gdarmont/go-jose: compact JWE format must have five parts")
	...
	if assert.NoError(t, err, "Error parsing signed-then-encrypted token.") {
		...
		if nested, err := tok7.Decrypt(testPrivRSAKey1); assert.NoError(t, err) {
			...
			assert.EqualError(t, nested.Claims(testPrivRSAKey2.Public()), "gdarmont/go-jose: error in cryptographic primitive")
		}
	}
	_, err = tok7.Decrypt(testPrivRSAKey2)
	assert.EqualError(t, err, "gdarmont/go-jose: error in cryptographic primitive")

	_, err = ParseSignedAndEncrypted(invalidSignedAndEncryptedToken)
	assert.EqualError(t, err, "gdarmont/go-jose/jwt: expected content type to be JWT (cty header)")
}
